...
image_original = Image.open("./ckpts/03/03_con_ori/attention/0-step/image.png")

sam_checkpoint = "/root/autodl-tmp/sam_vit_h_4b8939.pth"
model_type = "vit_h"


inputs_mask_1 = Image.open("./ckpts/03/03_con_ori/attention/0-step/final_masked0.png")
...
inputs_mask_1_np = np.array(tsfm_mask(inputs_mask_1.convert('L'))).transpose(0, 1).reshape(256,256) / 255
...
# plt.axis('on')
plt.savefig("./SAM/03/asset0/masked_image_output.png")

output_mask_np = np.squeeze(masks)
output_mask = Image.fromarray(output_mask_np)
output_mask.save("./SAM/03/asset0/mask_output.png")
...
output_image_masked.save("./SAM/03/asset0/sam_generated_masked_0.png")
